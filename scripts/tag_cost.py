...
after the fact. This reads the append-only record jobfeed/tag.py writes on every
...
from jobfeed.tag_telemetry import RUNS_LOG, load, price  # noqa: E402
...
def main() -> int:
    ...
    if not records:
        print(f"no records in {RUNS_LOG} for the last {args.days} days.\n"
              "tag_runs.jsonl is written by jobfeed/tag.py from 2026-08-20 on; an "
