...
Deliberately excluded:
  ...
  * the entire `applications` table (cover letters, ATS answers, screenshots)
...
def export(source: str, dest: str, with_descriptions: bool) -> dict:
    ...
    try:
        live = [r[1] for r in src.execute("PRAGMA table_info(seen_jobs)")]
        if not live:
            sys.exit("ABORT: no seen_jobs table in source database")
        ...
        try:
            ...
            rows = 0
            cur = src.execute(f"SELECT {collist} FROM seen_jobs")
            while True:
