...
# Append-only per-call cost/health record. tag_debug.log only ever records
# FAILURES, so "did tagging get more expensive" and "which transport actually
# ran" were unanswerable after the fact — on 2026-08-20 the only way to tell
# that a run had not used the configured provider was the mtime of .env.
...
def _json_to_pipe(out_text: str, expected_count: int) -> str:
    ...
    for i, row in enumerate(rows):
        if isinstance(row, dict):
            ...
            vals = list(row)
        vals = [str(v if v not in (None, "") else "-").replace("|", "/")
                for v in vals]
        if len(vals) != 13:
