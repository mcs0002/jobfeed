...
4. **Tag** (`tag.py`) — a structured model pass writes the
...
Scanner view reports the latest raw-board → pre-filter acquisition funnel,
...
.venv/bin/python main.py --verify --company "Qube Research & Technologies"  # fast smoke test
.venv/bin/python main.py --verify    # optional full 400+ source sweep; long-running
.venv/bin/python main.py --no-tag    # keyless first scan: scrape, enrich, store
# After configuring TAG_PROVIDER/API settings in .env:
.venv/bin/python main.py             # scrape, enrich, tag, store
```
...
verification; repeatable). The full verification waits for all light sources
and then checks heavy sources sequentially, so it can run for many minutes and
may print nothing between its opening line and the final per-source report.
...
[`KOCH_CAPTURE.md`](KOCH_CAPTURE.md) or accept one explicit Koch error while the
