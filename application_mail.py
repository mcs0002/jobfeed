...
Two rules carry the whole design, both borrowed from `application_limits.py`:
...
CANDIDATE_STATUSES = ("queued", "applied", "oa", "interview", "offer", "rejected")

# A status only ever moves forward. A later "thank you for applying" mail from a
# firm that has already invited him must not drag `interview` back to `applied`.
STATUS_ORDER = ("queued", "applied", "oa", "interview", "offer", "rejected")
...
# seen_jobs.company holds the scraper's source label, so one firm appears as
# "Citadel (HF)" or "UBS (Graduate Careers)". Citadel's own confirmation mail
# says Citadel and never HF, so the suffix has to come off before matching.
# Same rule the web app already applies to application caps at display time.
...
def _tokens(value: str) -> list[str]:
    bare = _PAREN_SUFFIX.sub("", value or "")
...
def fetch_messages(since: str, limit: int = 200, folder: str = "INBOX") -> list[dict]:
    ...
    second credential path: `application_account.py` already established this
...
def candidate_jobs(db: JobDB) -> list[dict]:
    ...
    cur = db.conn.execute(
        f"SELECT id, company, title, url, status FROM seen_jobs "
...
def _req_ids(job: dict) -> list[str]:
    return _REQ_RE.findall(f"{job.get('id', '')} {job.get('url', '')}".upper())
...
def is_forward(current: str, proposed: str) -> bool:
    return _rank(proposed) > _rank(current)
...
def audit(db: JobDB, stale_days: int = 21) -> list[str]:
    ...
    for job in candidate_jobs(db):
        ...
        if latest is None:
            continue
        if _rank(latest["proposed_status"]) > _rank(job["status"]):
            lines.append(
