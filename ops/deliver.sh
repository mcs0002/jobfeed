...
# weekly selfcheck. jobfeed/notify.py reads SMTP creds from .env.
alert() {
  .venv/bin/python -m jobfeed.notify "$1" "$2" >>"$LOG" 2>&1 || true
}
...
# Runs at 05:00, 11:00, 17:00 and 23:00 Europe/Berlin (2026-09-23).
# The live M1 launchd plist is authoritative. Only new roles are tagged.
...
rc=0
.venv/bin/python -m jobfeed >>"$LOG" 2>&1 || rc=$?
# rc=3 is the wedged-scan hard-exit (jobfeed/main.py os._exit(3)): the light batch timed
...
if (( rc == 3 )); then
  ...
  alert "job-scan: scan wedged (exit 3)" \
    "jobfeed/main.py hard-exited mid-run (light batch timed out). Results are partial; enrichment/tagging skipped. See deliver.log tail."
  exit "$rc"
elif (( rc != 0 )); then
  echo "[$(date -u +%FT%TZ)] scan failed (jobfeed/main.py exit $rc) — skipping enrichment" >>"$LOG"
  alert "job-scan: scan failed (exit $rc)" \
    "jobfeed/main.py exited $rc on the nightly run; enrichment/tagging skipped. See deliver.log tail."
  exit "$rc"
fi

# Backstop enrichment. Inline enrichment in jobfeed/main.py is best-effort with a tight
...
#
# Guarded like every step below: under set -e an unguarded failure here ended
# the script silently, skipping the re-tag, the inbox pass and the backup
# staleness check — the one alert that exists because backups fail silently.
.venv/bin/python -m jobfeed.enrich_descriptions --max-age-days 0 --limit 400 >>"$LOG" 2>&1 || {
  echo "[$(date -u +%FT%TZ)] description backstop failed (non-fatal)" >>"$LOG"
  alert "job-scan: description backstop failed" \
    "jobfeed/enrich_descriptions.py exited non-zero on the nightly run. The scan completed and the later steps still ran. See deliver.log tail."
}
...
.venv/bin/python -m jobfeed.backfill_tags --retag-missing-desc-facets >>"$LOG" 2>&1 || \
...
.venv/bin/python -m applications.mail --days 7 --apply >>"$LOG" 2>&1 || \
