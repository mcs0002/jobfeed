...
- Four body paragraphs: (1) the role + who they are + why they are interested, stated plainly in checkable facts, never opening on a thesis or an argument; (2) most relevant experience — {career_narrative}; (3) academic and quantitative foundation; (4) why this firm / desk specifically, plus availability, where at most one sentence of his own market view may appear, with its falsifier, or none at all.
...
def _letterhead_contact(profile: dict) -> str:
    """The contact rule under the name, in the CV's spelling.

    The CV and the letters carry a city and a LinkedIn, not a street address:
    they are documents an employer reads, not post he expects to receive. The
    profile's postal address stays where it belongs, on application forms.
    """
    addr = profile.get("address", {})
    location = profile.get("letterhead_location") or ", ".join(
        filter(None, [addr.get("city"), addr.get("country")])
    )
    linkedin = re.sub(r"^https?://(www\.)?", "", profile.get("linkedin", "") or "")
    return " · ".join(filter(None, [location, profile.get("email"),
                                    profile.get("phone"), linkedin]))


def render_pdf(body: str, out_path: str, profile: dict, letter_date: str) -> str:
    """Render the letter as a one-page US Letter business letter.

    Layout matches the CV and skills/cover-letter/letterhead.py so the two read
    as a set: name letterhead in caps over a contact rule, recipient block,
    date, bold subject, justified body.
    """
    ...
    name = profile.get("full_name", "")
    contact = _letterhead_contact(profile)

    head = ParagraphStyle("head", fontName="Helvetica-Bold", fontSize=13.5,
                          spaceAfter=3, leading=16, charSpace=1.2)
    sub = ParagraphStyle("sub", fontName="Helvetica", fontSize=7.4,
                         textColor="#55524a", leading=10)
    datestyle = ParagraphStyle("date", fontName="Helvetica", fontSize=9.2,
                               textColor="#55524a", leading=12, spaceAfter=12)
    subject = ParagraphStyle("subject", fontName="Helvetica-Bold", fontSize=10,
                             leading=13, spaceAfter=12)
    para = ParagraphStyle("para", fontName="Helvetica", fontSize=9.6,
                          alignment=TA_JUSTIFY, leading=14.4, spaceAfter=9,
                          textColor="#14140f")
    tight = ParagraphStyle("tight", fontName="Helvetica", fontSize=9.2,
                           leading=12.5, spaceAfter=0, textColor="#14140f")

    def esc(t: str) -> str:
        return (t.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;"))

    flow = [
        Paragraph(esc(name.upper()), head),
        Paragraph(esc(contact), sub),
        Spacer(1, 5),
        HRFlowable(width="100%", thickness=0.7, color="#b9b5aa"),
        Spacer(1, 16),
    ]

    blocks = [b for b in re.split(r"\n\s*\n", body.strip()) if b.strip()]
    lined = [[esc(ln.strip()) for ln in b.splitlines() if ln.strip()] for b in blocks]

    index = 0
    # Recipient block: the opening block when it runs to several lines.
    if lined and len(lined[0]) > 1 and not lined[0][0].startswith("Dear"):
        for line in lined[0]:
            flow.append(Paragraph(line, tight))
        index = 1
    flow.append(Spacer(1, 12))
    flow.append(Paragraph(esc(letter_date), datestyle))
    # Subject: a single line standing on its own before the salutation.
    if index < len(lined) and len(lined[index]) == 1 and not lined[index][0].startswith("Dear"):
        flow.append(Paragraph(lined[index][0], subject))
        index += 1
    for lines in lined[index:]:
...
def main() -> int:
    ...
    today = date.today()
    place_date = f"{today.day} {today.strftime('%B %Y')}"
    folder = os.path.join(
