...
def fmt(text, title=None, company=None):
    return str(format_description(text, title, company))
...
def test_societe_generale_section_headings_detected():
    src = (
        "Profile required\n\n• Degree in finance\n\n"
        "Behavioral Skills\n\n• Client focus\n\n"
        "Business insight\n\nOur culture values teamwork."
    )
    out = fmt(src)
    assert "<h4>Profile required</h4>" in out
    assert "<h4>Behavioral Skills</h4>" in out
    assert "<h4>Business insight</h4>" in out


def test_legacy_societe_generale_runon_gets_source_scoped_boundaries():
    src = (
        "ResponsibilitiesThis is the role.The candidate supports clients."
        "Profile requiredBachelor's degree.Why join usBusiness insightOur culture."
    )
    out = fmt(src, company="Société Générale")
    assert "<h4>Responsibilities</h4>" in out
    assert "<p>This is the role.</p>" in out
    assert "<p>The candidate supports clients.</p>" in out
    assert "<h4>Profile required</h4>" in out
    assert "<h4>Why join us</h4>" in out
    assert "<h4>Business insight</h4>" in out


def test_legacy_runon_repair_does_not_touch_other_companies():
    src = "ResponsibilitiesThis is joined.The next sentence is joined."
    out = fmt(src, company="Other Bank")
    assert "<h4>Responsibilities</h4>" not in out
