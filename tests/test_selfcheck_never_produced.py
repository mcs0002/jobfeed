...
def _monitors_absent():
    """Make the three best-effort monitors fail the way a missing one does, so
    the suite can never sample the live M1 database or apply URLs."""
    stack = contextlib.ExitStack()
    for target in ("jobfeed.description_health.degraded_sources",
                   "jobfeed.tag_health.other_heavy_sources",
                   "jobfeed.link_health.dead_sources"):
        stack.enter_context(mock.patch(target, side_effect=ImportError("absent in test")))
    return stack
...
class NeverProducedTests(unittest.TestCase):
    def _run(self, counts, state_path):
        ...
        with mock.patch.object(selfcheck.subprocess, "run", return_value=fake), \
            ...
            # DB/network monitors are best-effort production integrations, not
            # part of this state-machine test. Force all three absent so the
            # suite cannot sample the live M1 database or apply URLs.
            with _monitors_absent():
                selfcheck.main()
...
class BailPathAlertTests(unittest.TestCase):
    ...
    def _run_bail(self, returncode, stdout, stderr, state_path, env_email="1"):
        ...
        with mock.patch.object(selfcheck.subprocess, "run", return_value=fake), \
             ...
             _monitors_absent(), \
             mock.patch("builtins.print"), \
             mock.patch("jobfeed.notify.send_alert", side_effect=_fake_send):
            rc = selfcheck.main()
        return rc, sent
    ...
    def test_bail_alert_failure_does_not_raise(self):
        ...
        with TemporaryDirectory() as d:
            ...
            with mock.patch.object(selfcheck.subprocess, "run", return_value=fake), \
                 ...
                 mock.patch("jobfeed.notify.send_alert", broken_notify.send_alert), \
                 _monitors_absent(), \
                 mock.patch("builtins.print"):
                rc = selfcheck.main()
        self.assertEqual(rc, 2)
