...
class DachInsurerNoiseTests(unittest.TestCase):
    """The 2026-08-03 DACH insurer expansion wired ~26 whole-company boards, so
    German/Swiss agency-sales and vocational noise started flooding the store
    (August's area='other' share went 48% -> 61%). These buckets shed it.
    Every term below was dry-run against the live board first — the only
    non-'other' rows they touch are retail insurance agency roles."""

    # --- CH/AT vocational tracks (pre-degree, always dropped) ---

    def test_schnupperlehre_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Schnupperlehre als Elektroinstallateur/in EFZ (m/w/d)"}))

    def test_lehrstelle_drops(self):
        self.assertFalse(is_relevant(
            {"title": "KV Lehrstelle EFZ 2027 Generalagentur Limmattal"}))

    def test_lernende_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Lernende/r Elektroinstallateur/in EFZ - 2027"}))

    def test_schulpraktikum_drops(self):
        # Distinct spelling from 'Schülerpraktikum', which was already listed.
        self.assertFalse(is_relevant(
            {"title": "Schulpraktikum IT im Zeitraum 05.10.-16.10.2026"}))

    def test_efz_is_word_bounded(self):
        # 'efz' is <=3 chars so it matches standalone only and cannot appear
        # as a substring of an unrelated word.
        self.assertFalse(is_relevant({"title": "Kaufmann EFZ"}))
        self.assertTrue(is_relevant({"title": "Analyst Refzone Markets"}))

    # --- tied-agency insurance distribution ---

    def test_generalagentur_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Versicherungsberater:in für die Generalagentur Aarau"}))

    def test_vorsorgeberater_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Vorsorgeberater:in Vorsorge & Vermögen Zug"}))

    def test_aussendienst_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Angestellter Vermittler (d/m/w) im Außendienst Raum Berlin"}))

    def test_agency_assistant_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Vertriebsassistenz/Büroassistenz (m/w/d) in einer Versicherungsagentur"}))

    def test_versicherungsfachmann_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Versicherungsfachmann (m/w/d) als Agenturinhaber (m/w/d)"}))

    # --- insurance policy/claims administration ---

    def test_claims_and_policy_admin_drop(self):
        for title in ("Mitarbeiter (m/w/d) für die Schadenbearbeitung",
                      "Schadenspezialist (Wmd) Generalagentur Wil SG",
                      "Mitarbeiter (m/w/d) Vertragsservice Unfallversicherung",
                      "Mitarbeiter (m/w/d) Leistungsservice Krankenversicherung"):
            with self.subTest(title=title):
                self.assertFalse(is_relevant({"title": title}))

    def test_rechtsreferendariat_drops(self):
        self.assertFalse(is_relevant(
            {"title": "Rechtsreferendariat für die Wahlstation - Münster"}))

    # --- what must SURVIVE: the false positives the dry-run caught ---

    def test_bare_sachbearbeiter_is_not_dropped(self):
        """'sachbearbeiter' was rejected as a term: it hits real finance roles.
        Only the claims/policy compounds are dropped."""
        for title in ("Sachbearbeiter Quellensteuer (m/w/d)",
                      "Accountant/Sachbearbeiter Buchhaltung (All Genders)",
                      "Sachbearbeiter:in Steuern (m/w/d)"):
            with self.subTest(title=title):
                self.assertTrue(is_relevant({"title": title}))

    def test_risikopruefer_is_not_dropped(self):
        # Insurance underwriting shades into actuarial work — left in scope.
        self.assertTrue(is_relevant({"title": "Risikoprüfer Leben (m/w/d)"}))

    def test_kundenberater_firmenkunden_survives(self):
        """Bare 'kundenberater' is deliberately not a drop term — the corporate
        banking form of the title is wanted."""
        self.assertTrue(is_relevant({"title": "Kundenberater Firmenkunden"}))

    def test_institutional_dach_roles_survive(self):
        for title in ("Portfoliomanager (m/w/d) Fixed Income",
                      "(Junior) Investment Manager (d/m/w)",
                      "Aktuar (d/m/w) Produktintegration Leben",
                      "Inhouse Consultant Kapitalanlage (m/w/d)",
                      "Junior Risk Analyst (f/m/d)"):
            with self.subTest(title=title):
                self.assertTrue(is_relevant({"title": title}))
