...
from jobfeed import description_health as dh
...
class DescriptionHealthTests(unittest.TestCase):
    def setUp(self):
        ...
        conn.execute("CREATE TABLE jobs_with_description (company TEXT, description TEXT, "
                     "delisted_at TEXT)")
        rows = (
        ...
        conn.executemany("INSERT INTO jobs_with_description VALUES (?,?,?)", rows)
        conn.commit()
        conn.close()
    ...
    def test_small_source_not_flagged(self):
        ...
        conn = sqlite3.connect(self.db)
        conn.executemany("INSERT INTO jobs_with_description VALUES (?,?,?)",
                         [("TinyCo", "y" * 50, None)] * 3)
