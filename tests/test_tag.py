...
from jobfeed import claude_cli
from jobfeed import tag
from jobfeed.tag import parse_response, _coerce
...
def setUpModule():
    ...
    _GUARDS = [
        ...
        patch.dict(os.environ, {
            ...
            "TAG_API_MODEL": "",
            "TAG_API_EXTRA": "",
            # Production sets json_rows + an addendum; left unpinned, the M1's
            # .env turned every pipe-format fake response into a parse failure.
            "TAG_API_FORMAT": "",
            "TAG_RUBRIC_ADDENDUM": "",
            "TAG_API_MAX_TOKENS": "",
        ...
        # Transport retries back off for real seconds; the suite must not.
        patch.object(tag, "API_RETRY_DELAYS", (0, 0)),
    ]
    for g in _GUARDS:
        g.start()
...
class ParseResponseTests(unittest.TestCase):
    ...
    def test_economics_is_a_valid_area(self):
        out = parse_response(
            "0|economics|-|analyst|job|Frankfurt|Germany|Europe|onsite|-|0|master|-\n",
            expected_count=1)
        self.assertEqual(out[0]["area"], "economics")
        self.assertEqual(out[0]["desk"], "")
...
class TagJobsTests(unittest.TestCase):
    def test_tags_applied_in_place(self):
        ...
        with patch.object(tag, "claude_bin", return_value="/fake/claude"), \
    ...
    def test_no_cli_leaves_blank_tags(self):
        jobs = [{"title": "FX Trader", "company": "GS", "location": "London"}]
        with patch.object(tag, "claude_bin", return_value=None):
            tag.tag_jobs(jobs)
...
class TestWarmAuth(unittest.TestCase):
    ...
    def test_failure_does_not_retry(self):
        """A failed warm is not retried, and is not reported as a success.

        The second assertion used to read assertTrue: the flag recorded that an
        attempt had happened, not how it went, so every call after a failure
        claimed the CLI was warm. Nothing branched on it, which is the only
        reason it never showed up as a bug."""
        with patch.object(claude_cli.subprocess, "run",
    ...
    def test_nonzero_exit_is_a_failure(self):
        """A CLI that runs and refuses is as dead as one that will not start."""
        class FailProc:
            returncode = 1
            stdout = ""
            stderr = "OAuth session expired"

        with patch.object(claude_cli.subprocess, "run",
                          return_value=FailProc()) as run:
            self.assertFalse(
                claude_cli.warm_auth("/fake/claude", "claude-haiku-4-5"))
            self.assertFalse(
                claude_cli.warm_auth("/fake/claude", "claude-haiku-4-5"))
...
class ApiTransportTests(unittest.TestCase):
    ...
    def test_openai_is_primary_and_cli_is_never_called(self):
        ...
        with patch.dict(os.environ, self.CFG, clear=False), \
             ...
             patch.object(tag, "claude_bin", return_value="/fake/claude"), \
    ...
    def test_openai_primary_works_without_claude_installed(self):
        ...
        with patch.dict(os.environ, self.CFG, clear=False), \
             ...
             patch.object(tag, "claude_bin", return_value=None), \
             patch.object(tag, "warm_auth") as warm, \
             patch.object(tag, "_tag_batch",
                          side_effect=AssertionError("CLI must not be called")):
            tag.tag_jobs(jobs)
    ...
    def test_structured_mode_sends_schema_and_reuses_parser(self):
        jobs = self._jobs(3)
        posted = {}
        rows = [[str(i), "markets", "trading", "analyst", "job", "London",
                 "United Kingdom", "Europe", "onsite", "-", "0", "-", "-"]
                for i in range(3)]
        ...
        cfg = dict(self.CFG)
        cfg.update({"TAG_API_FORMAT": "json_rows",
                    "TAG_RUBRIC_ADDENDUM": "exclusions"})
        import requests
        with patch.dict(os.environ, cfg, clear=False), \
             patch.object(requests, "post", fake_post), \
             patch.object(tag, "claude_bin", return_value=None):
            tag.tag_jobs(jobs)

        schema = posted["response_format"]["json_schema"]["schema"]
        self.assertEqual(schema["properties"]["rows"]["minItems"], 3)
        self.assertEqual(schema["properties"]["rows"]["maxItems"], 3)
        self.assertIn("BOUNDARY CHECK", posted["messages"][0]["content"])
        self.assertTrue(all(j["area"] == "markets" for j in jobs))

    def test_structured_rows_tolerate_newlines_inside_fields(self):
        row = ["0", "economics", "-\n", "analyst", "graduate-programme",
               "Washington, DC", "United States", "Americas", "onsite", "-\n",
               "0", "master", " -"]
        pipe = tag._json_to_pipe(json.dumps({"rows": [row]}), 1)
        self.assertEqual(pipe.count("\n"), 0)
        out = tag.parse_response(pipe, expected_count=1)
        self.assertEqual(out[0]["area"], "economics")
        self.assertEqual(out[0]["loc_city"], "Washington, DC")

    def test_structured_rows_reject_wrong_count_shape_or_index(self):
        valid = [str(i) for i in range(13)]
        valid[0] = "0"
        with self.assertRaises(ValueError):
            tag._json_to_pipe(json.dumps({"rows": []}), 1)
        with self.assertRaises(ValueError):
            tag._json_to_pipe(json.dumps({"rows": [["0"]]}), 1)
        invalid_index = list(valid)
        invalid_index[0] = "4"
        with self.assertRaises(ValueError):
            tag._json_to_pipe(json.dumps({"rows": [invalid_index]}), 1)
        self.assertEqual(len(tag._json_to_pipe(
            json.dumps({"rows": [valid]}), 1).split("|")), 13)

    def test_structured_prompt_changes_rubric_provenance(self):
        pipe_cfg = dict(self.CFG)
        pipe_cfg.update({"TAG_API_FORMAT": "pipe", "TAG_RUBRIC_ADDENDUM": "none"})
        json_cfg = dict(self.CFG)
        json_cfg["TAG_API_FORMAT"] = "json_rows"
        with patch.dict(os.environ, pipe_cfg, clear=False):
            pipe_version = tag.tag_provenance()["rubric_version"]
        with patch.dict(os.environ, json_cfg, clear=False):
            json_version = tag.tag_provenance()["rubric_version"]
        self.assertNotEqual(pipe_version, json_version)
    ...
    def _status(self, code):
        ...
        class R:
            ...
            @staticmethod
            def raise_for_status():
                raise requests.HTTPError(f"{code} error")
            @staticmethod
            def json(): return {}
        return R()

    def test_transient_failures_are_retried_without_leaving_the_primary(self):
        """One 503 or timeout used to mark the primary dead for the whole run
        and hand the rest of the night to the subscription CLI."""
        jobs = self._jobs(3)
        import requests
        replies = [requests.Timeout("read timed out"), self._status(503),
                   self._resp(3)]
        ...
        self.assertFalse(tag.LAST_RUN_HEALTH.get("api_transport_down"))

    def test_persistent_transient_failure_still_falls_back(self):
        jobs = self._jobs(3)
        import requests
        calls = []
        ...
        with patch.dict(os.environ, self.CFG, clear=False), \
             ...
             patch.object(tag, "_tag_batch", side_effect=cli):
            tag.tag_jobs(jobs)
        self.assertEqual(len(calls), 1 + len(tag.API_RETRY_DELAYS))
        self.assertTrue(tag._api_transport_is_dead())
        self.assertTrue(all(j["area"] == "markets" for j in jobs))
    ...
    def test_permanent_error_is_not_retried(self):
        ...
        with self.assertRaises(requests.HTTPError):
            with patch.object(requests, "post", fake_post):
                tag._post_json("https://x", headers={}, body={}, label="T")
...
class WaveConcurrencyTests(unittest.TestCase):
    """API batches go out in concurrent waves; the CLI path stays sequential."""

    CFG = ApiTransportTests.CFG
    ...
    def test_api_batches_overlap_and_health_merges(self):
        jobs = self._jobs(tag.BATCH_SIZE * 5)          # 5 batches: waves of 4 + 1
        state, enter = self._probe()
        import requests

        def fake_post(url, **kw):
            enter()
            n = kw["json"]["messages"][1]["content"].count("===") + 1
            return ApiTransportTests._resp(None, n, cached=100)
        ...
        self.assertGreater(state["peak"], 1, "batches never overlapped")
        self.assertLessEqual(state["peak"], tag.TAG_WORKERS)
        self.assertTrue(all(j["area"] == "markets" for j in jobs))
        h = tag.LAST_RUN_HEALTH
        self.assertEqual(h["batches_total"], 5)
        self.assertEqual(h["tokens_in"], 5000)
        self.assertEqual(h["tokens_cached"], 500)
        self.assertEqual(h["jobs_tagged"], len(jobs))

    def test_cli_batches_stay_sequential(self):
        jobs = self._jobs(tag.BATCH_SIZE * 3)
        state, enter = self._probe(delay=0.01)
