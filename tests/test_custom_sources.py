...
class JibeCompletenessTests(unittest.TestCase):
    ...
    @patch("scrapers.jibe.make_session")
    def test_repeated_page_fails_instead_of_inflating_total(self, make_session):
        repeated = FakeResponse({
            "totalCount": 2,
            "jobs": [{"data": {"req_id": "10", "title": "Analyst"}}],
        })
        make_session.return_value.get.return_value = repeated
        with self.assertRaises(RuntimeError) as ctx:
            jibe.scrape({"base_url": "https://careers.example.com"})
        self.assertIn("no new posting ids", str(ctx.exception))


class SmartRecruitersCompletenessTests(unittest.TestCase):
    @patch("scrapers.smartrecruiters.make_session")
    def test_repeated_offset_page_fails_on_unique_ids(self, make_session):
        content = [{
            "id": str(i), "name": f"Role {i}",
            "ref": f"https://jobs.example/{i}", "location": {},
            "releasedDate": "",
        } for i in range(100)]
        make_session.return_value.get.return_value = FakeResponse({
            "totalFound": 200, "content": content,
        })
        with self.assertRaises(RuntimeError) as ctx:
            smartrecruiters.scrape("example")
        self.assertIn("no new posting ids", str(ctx.exception))
