...
class SchemaMigrationTests(unittest.TestCase):
    ...
    def test_legacy_db_gains_new_columns_and_keeps_rows(self):
        ...
        # Text lives in its own table since 2026-09-23.
        self.assertNotIn("description", columns)
        self.assertIsNone(db.get_job("legacy-1")["description"])
        cur = db.conn.execute(
...
class DescriptionTests(unittest.TestCase):
    ...
    def test_mark_seen_stores_description_when_supplied(self):
        ...
        row = self.db.conn.execute(
            "SELECT description, description_fetched_at FROM jobs_with_description "
            "WHERE id = ?", ("a1",),
        ).fetchone()
    ...
    def test_set_description_fills_in_after_the_fact(self):
        ...
        self.assertTrue(ok)
        self.assertEqual(self.db.get_job("a3")["description"],
                         "Cover the rates book.")
...
class EnrichQueueTests(unittest.TestCase):
    ...
    def _queued(self) -> set[str]:
        return {r["id"] for r in self.db.jobs_missing_description(limit=10)}

    def test_delisted_rows_leave_the_queue(self):
        self.db.mark_delisted(["gh_3"])
        self.assertNotIn("gh_3", self._queued())
        # Reappearing on the board clears delisted_at and re-queues it.
        self.db.mark_seen("gh_3", company="HRT", title="C", url="u3")
        self.assertIn("gh_3", self._queued())

    def test_miss_backs_off_exponentially(self):
        waits = []
        for _ in range(8):
            self.db.record_description_miss("gh_3")
            last, nxt = self.db.conn.execute(
                "SELECT last_attempt_at, next_attempt_at FROM description_attempts "
                "WHERE job_id = 'gh_3'").fetchone()
            waits.append(round((datetime.fromisoformat(nxt)
                                - datetime.fromisoformat(last)).total_seconds() / 86400))
        self.assertEqual(waits, [1, 2, 4, 8, 16, 30, 30, 30])
        self.assertNotIn("gh_3", self._queued())

    def test_due_miss_is_retried_and_success_clears_it(self):
        self.db.record_description_miss("gh_3")
        self.db.conn.execute(
            "UPDATE description_attempts SET next_attempt_at = '2000-01-01'")
        self.assertIn("gh_3", self._queued())
        self.db.set_description("gh_3", "full text")
        self.assertEqual(self.db.conn.execute(
            "SELECT count(*) FROM description_attempts").fetchone()[0], 0)
...
class TagsAndFetchTests(unittest.TestCase):
    ...
    def test_human_evaluation_freezes_prediction_and_corrects_job(self):
        self.db.set_tags("a", area="other", seniority="manager",
                         provider="api", model="deepseek-test",
                         rubric_version="rubric-1")
        self.assertTrue(self.db.record_tag_evaluation(
            "a", bucket="other", area="markets", desk="trading",
            seniority="analyst", job_type="job"))
        job = self.db.get_job("a")
        self.assertEqual((job["area"], job["seniority"]), ("markets", "analyst"))
        summary = self.db.tag_evaluation_summary()
        self.assertEqual(summary["total"], 1)
        self.assertEqual(summary["false_manager"], 1)
        self.assertEqual(summary["false_other"], 1)
        self.assertEqual(summary["models"][0]["model"], "deepseek-test")
        # Re-editing human truth must not replace the frozen model prediction.
        self.db.record_tag_evaluation("a", bucket="other", area="ibd", desk="",
                                      seniority="analyst", job_type="job")
        baseline = self.db.conn.execute(
            "SELECT baseline_area FROM tag_evaluations WHERE job_id='a'").fetchone()[0]
        self.assertEqual(baseline, "other")

    def test_application_funnel(self):
        self.db.set_tags("a", area="markets")
        self.db.set_tags("b", area="ibd")
        self.db.set_status("a", "applied")
        self.db.set_status("a", "interview")
        self.db.set_status("b", "applied")
        rows = {r["label"]: r for r in self.db.application_funnel("area")}
        self.assertEqual(rows["markets"]["response_rate"], 100)
        self.assertEqual(rows["ibd"]["response_rate"], 0)
    ...
    def test_prune_preserves_applied_descriptions(self):
        ...
        for jid in ("a", "b"):
            self.db.conn.execute(
                "UPDATE seen_jobs SET first_seen = ? WHERE id = ?", (old, jid))
            self.db.set_description(jid, "body")
        self.db.conn.commit()
...
class FetchDedupTests(unittest.TestCase):
    ...
    def test_shared_board_url_different_titles_stay_separate(self):
        """A board-level fallback URL is not a posting identity."""
        self.db.mark_seen("a", company="Co", title="Quant Analyst",
                          url="https://co.com/careers")
        self.db.mark_seen("b", company="Co", title="Portfolio Analyst",
                          url="https://co.com/careers")
        self.assertEqual(len(self.db.fetch_jobs(company="Co")), 2)

    def test_same_url_across_companies_stays_separate(self):
        self.db.mark_seen("a", company="Co A", title="Analyst",
                          url="https://shared-ats.example/jobs")
        self.db.mark_seen("b", company="Co B", title="Analyst",
                          url="https://shared-ats.example/jobs")
        self.assertEqual(len(self.db.fetch_jobs()), 2)
...
class IdChurnTests(unittest.TestCase):
    ...
    def test_find_active_by_url_respects_company_and_title(self):
        board = "https://co.com/careers"
        self.db.mark_seen("one", company="Co", title="Quant Analyst", url=board)
        self.assertEqual(
            self.db.find_active_by_url(
                board, "fresh", company="Co", title="Quant Analyst"),
            "one",
        )
        self.assertIsNone(self.db.find_active_by_url(
            board, "fresh", company="Co", title="Portfolio Analyst"))
        self.assertIsNone(self.db.find_active_by_url(
            board, "fresh", company="Other", title="Quant Analyst"))
...
class DeadlineStorageTests(unittest.TestCase):
    """The S12 closing filter shipped before any scraper could fill `deadline`,
    so every row in the live database had a NULL one until Universia."""
    ...
    def _deadline(self, job_id="j1"):
        return self.db.conn.execute(
            "SELECT deadline FROM seen_jobs WHERE id = ?", (job_id,)
        ).fetchone()[0]

    def test_a_stated_deadline_is_stored_and_a_missing_one_stays_null(self):
        self.db.mark_seen("j1", company="C", title="Graduate Programme",
                          deadline="2026-10-04")
        self.db.mark_seen("j2", company="C", title="Rolling role")
        self.assertEqual("2026-10-04", self._deadline("j1"))
        self.assertIsNone(self._deadline("j2"))

    def test_a_later_scrape_that_omits_the_date_does_not_clear_it(self):
        """A board that stops repeating a deadline has not withdrawn it."""
        self.db.mark_seen("j1", company="C", title="T", deadline="2026-10-04")
        self.db.mark_seen("j1", company="C", title="T")
        self.assertEqual("2026-10-04", self._deadline())

    def test_a_board_that_states_a_date_later_reaches_an_existing_row(self):
        """mark_seen only runs for brand-new rows, so without this the 27
        Santander programmes kept the NULL deadline they were created with."""
        self.db.mark_seen("j1", company="C", title="T")
        self.assertTrue(self.db.set_deadline("j1", "2026-10-04"))
        self.assertEqual("2026-10-04", self._deadline())
        # Moving it is a real change; repeating it is not a write.
        self.assertTrue(self.db.set_deadline("j1", "2026-11-30"))
        self.assertFalse(self.db.set_deadline("j1", "2026-11-30"))
        self.assertFalse(self.db.set_deadline("j1", ""))
        self.assertEqual("2026-11-30", self._deadline())
