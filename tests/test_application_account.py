...
from applications.account import _domain_ok, _earlier_account_mail, credentials, verification
from jobfeed.db import JobDB


@mock.patch("applications.account._earlier_account_mail", new=lambda host, before: None)
class AccountBrokerTests(unittest.TestCase):
    def test_an_ats_sender_may_link_to_an_ats_host(self):
        ...
        from applications.account import _domain_ok
    ...
    @mock.patch("applications.account._profile_email", return_value="applicant@example.test")
    @mock.patch("applications.account.subprocess.run")
    def test_security_answers_are_generated_once_and_survive_a_reset(self, run, _email):
        ...
        with mock.patch("applications.account._keychain",
    ...
    def test_mail_from_the_firm_before_the_workflow_marks_an_earlier_account(self):
        ...
        with mock.patch.object(broker.subprocess, "run") as run:
            ...
            # Fidelity 2026-09-19: Oleeo tenants share tal.net.
            self.assertIsNone(_earlier_account_mail("fidelityinternational.tal.net", None))
    ...
    @mock.patch("applications.account._profile_email", return_value="applicant@example.test")
    @mock.patch("applications.account._keychain", return_value="StoredPassword1!")
    @mock.patch("applications.account.subprocess.run")
    def test_a_stored_password_is_not_a_confirmed_account(self, run, _keychain, _email):
        ...
        from applications.account import mark_verified
        mark_verified("apply_12345678")
        self.assertTrue(credentials("apply_12345678")["confirmed"])

    @mock.patch("applications.account._profile_email", return_value="applicant@example.test")
    @mock.patch("applications.account._keychain", return_value=None)
    @mock.patch("applications.account.subprocess.run")
    ...
    @mock.patch("applications.account._profile_email", return_value="applicant@example.test")
    @mock.patch("applications.account._keychain", return_value="OldPassword1!")
    @mock.patch("applications.account.subprocess.run")
