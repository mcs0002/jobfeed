...
The request goes in a fixed file instead, the same trick the other
fixed-request commands use, so one grant covers
...
from applications.handoff import read_fixed_request, workflow_token
from jobfeed.envfile import load_dotenv
from jobfeed.db import AGENT_WORKFLOW_STATES
...
def _read_request(path: Path) -> dict:
    """The agent-written request, checked as a file before it is read as JSON.

    `FIXED_REQUEST` lives in world-writable /private/tmp, because the invariant
    command text is what Antigravity's permission grant covers and a per-run
    path would mean a fresh prompt each time. The path cannot move, so the read
    is hardened instead: `read_fixed_request` refuses a symlink, a non-regular
    file, a file owned by someone else and a file another account can rewrite,
    all checked against the open descriptor. `applications/launcher.py` has
    always done this for the request the web service writes; this one, written
    by the agent, went straight to `path.read_text()`."""
    obj = read_fixed_request(path)
    allowed = {"workflow_id", "run_id", "status", "detail", "application_limit"}
    if set(obj) - allowed:
        raise ValueError(
            "request must hold only workflow_id, run_id, status, detail and application_limit")
    ...
    if run_id and (not run_id.startswith(RUN_PREFIX) or not run_id[4:].isalnum()):
