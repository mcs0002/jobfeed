...
Usage:
    python -m applications.limits                     # every configured firm
    python -m applications.limits --company Barclays  # one firm
    python -m applications.limits --company "UBS (Graduate Careers)" \
        --url https://www.ubs.com/global/en/careers/early-careers/faq.html
    python -m applications.limits --refresh-days 90 --limit 50
    LIMITS_BROWSERS=5 python -m applications.limits --refresh-days 0
...
from jobfeed import tag as _tag                                      # noqa: E402
...
# minute and would still be running when the nightly scan starts.
...
def load_targets(only: str | None, include_all: bool) -> list[dict]:
    ...
    for t in targets:
        ...
        # per docs/GRAD_SCHEMES.md it marks firms whose roles the ATS scrape MISSES
