...
from applications.handoff import read_fixed_request, safe_application_url
...
ATS_DOMAINS = ("workday.com", "myworkdayjobs.com", "myworkdaysite.com",
               ...
               # Every other multi-tenant host in targets.json. Fidelity's
               # `fidelityinternational.tal.net` read as brand "tal" and matched
               # Perella Weinberg's Oleeo confirmation, so a firm he had never
               # applied to was reported as an existing account (2026-09-19).
               "tal.net", "successfactors.eu", "oraclecloud.eu", "personio.de",
               "pinpointhq.com", "workable.com", "csod.com", "avature.net",
               "recruitee.com", "teamtailor.com", "umantis.com", "jobvite.com",
               "bamboohr.com")
...
def credentials(workflow_id: str, *, rotate: bool = False) -> dict:
    ...
    # Withholding it and typing over CDP instead (applications/password.py) put a
    ...
    # applications/password.py is kept for the day that consent can be held open.
...
def _fixed_request() -> tuple[str, str]:
    # This is the credential broker, so a request planted in /private/tmp is
    # the worst place for a plain read_text(): see read_fixed_request.
    obj = read_fixed_request(FIXED_REQUEST, max_bytes=4096)
    if set(obj) != {"workflow_id", "action"}:
