...
# tagger's excerpt builder (clean_text via jobfeed/tag.py), and every already-stored
# row are fixed without a re-scrape.
_LITERAL_ESCAPES = (("\\r\\n", "\n"), ("\\n", "\n"), ("\\r", "\n"), ("\\t", " "))

# SG's old JSON-LD flattened every block boundary. Most active rows are
# backfilled from the structured visible DOM by societegenerale_enrich, but a
# role can disappear between the listing scan and that repair (the detail page
# then returns 404/410). Keep those stale rows readable without applying an
# uppercase-boundary heuristic to other companies' prose.
_SG_COMPANY_RE = re.compile(r"^soci(?:é|e)té générale", re.IGNORECASE)
_SG_LEGACY_HEADINGS = (
    "Responsibilities",
    "Profile required",
    "Behavioral Skills",
...
_JOINED_SENTENCE_RE = re.compile(r"(?<=[.!?])(?=[A-ZÀ-ÖØ-Þ])")
...
def _restore_socgen_legacy(text: str, company: str | None) -> str:
    """Recover safe boundaries in single-line SG JSON-LD kept after a 404."""
    if "\n" in text or not _SG_COMPANY_RE.match((company or "").strip()):
        return text
    restored = text
    for heading in _SG_LEGACY_HEADINGS:
        restored = restored.replace(heading, f"\n\n{heading}\n\n")
    restored = _JOINED_SENTENCE_RE.sub("\n\n", restored)
...
def clean_text(text: str | None, title: str | None = None) -> str:
    ...
    cleaner but return joined text instead of HTML. Shared with jobfeed/tag.py's excerpt
...
def format_description(text: str | None, title: str | None = None,
                       company: str | None = None) -> Markup:
    ...
    lines = _clean_lines(_restore_socgen_legacy(text, company), title)
