...
class UniquePagination:
    """Shared completeness contract for paginated ATS responses.

    Counts posting identities, not rows. A non-empty page adding no identities
    is a repeated-page loop and fails immediately; ``finish`` rejects a silent
    partial before downstream delisting can interpret it as a smaller board.
    """
    def __init__(self, source: str):
        self.source = source
        self.ids: set[str] = set()

    def add_page(self, ids, cursor: str | int = "?") -> None:
        page = {str(i) for i in ids if i is not None and str(i)}
        before = len(self.ids)
        self.ids.update(page)
        if page and len(self.ids) == before:
            raise RuntimeError(
                f"{self.source}: cursor={cursor} repeated with no new posting ids")

    def finish(self, total: int | None, band: float = 0.9) -> int:
        assert_complete(len(self.ids), total, self.source, band)
        return len(self.ids)
