...
def scrape(
    ...
    jobs = {}
    rows_parsed = 0  # raw cards with a job link, pre-dedup (Bloomberg lists
    # ~20 of its ~400 reqs twice, so unique ids < reported items is normal)
    ...
    # Judge completeness on RAW parsed rows, not unique ids — boards can list
    # one requisition twice, and those dups are not missing data.
    if total is not None and rows_parsed != total:
        if rows_parsed < 0.9 * total:
            raise RuntimeError(
                f"Avature reported {total} jobs but parsed {rows_parsed}")
        print(
            f"WARN Avature reported {total} jobs but parsed {rows_parsed}; "
