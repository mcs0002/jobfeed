...
# imported inside the one adapter that can still route to it (_workday's 422
# fallback).
...
def _metzler(company):
    return metzler.scrape()
...
def _universia(company):
    return universia.scrape(company["universia"])
...
def _talnet(company):
    return talnet.scrape(
        ...
        id_scope=company.get("talnet_id_scope", ""),
    )
...
def _nuernberger(company):
    return nuernberger.scrape(company.get("nuernberger"))


def _ww(company):
    return ww.scrape(company.get("ww"))


def _provinzial(company):
    return provinzial.scrape(company.get("provinzial"))


def _alh(company):
    return alh.scrape(company.get("alh"))


def _vig(company):
    return vig.scrape(company.get("vig"))


def _dvinci(company):
    return dvinci.scrape(company["dvinci"])


def _intervieweb_career(company):
    return intervieweb_career.scrape(
        company["career_url"],
        slug=company.get("name", "intervieweb").lower().replace(" ", "-"),
    )
...
def _icims(company):
    return icims.scrape(company["icims"]["base_url"],
                        id_scope=company["icims"].get("id_scope", ""))
...
def _manual(company):
    # "manual" = wanted firm that can't be scraped reliably (see jobfeed/manual_check.py).
...
HANDLERS: dict[str, callable] = {
    ...
    "intervieweb_career": _intervieweb_career,
    "nuernberger": _nuernberger,
    "ww": _ww,
    "provinzial": _provinzial,
    "alh": _alh,
    "vig": _vig,
    "dvinci": _dvinci,
