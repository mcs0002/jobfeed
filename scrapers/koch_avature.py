...
jobfeed/main.py catches per-company exceptions, so the rest of the run still
...
``docs/KOCH_CAPTURE.md`` (60-second capture procedure).
...
def scrape(config: dict) -> list[dict]:
    ...
    if loaded is None:
        raise RuntimeError(
            f"KOCH_COOKIES_MISSING path={path} — recapture per docs/KOCH_CAPTURE.md"
        )
    ...
    # captured manually from the filter sidebar, see docs/KOCH_CAPTURE.md.
    ...
    for page in range(max_pages):
        ...
        if _challenged(response):
            raise RuntimeError(
                ...
                "secrets/koch_cookies.json (see docs/KOCH_CAPTURE.md)"
            )
