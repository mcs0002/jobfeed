...
def _token(session, base_url, company):
    ...
    if resp.is_redirect and base_url not in resp.headers.get("Location", ""):
        # Tenant has a vanity career-site URL configured and the JOB_SEARCH
        # view 302s to it. If that domain is dead (Mizuho EMEA decommissioned
        # careers.mizuhoemea.com ~July 2026 while the SF tenant stayed live),
        # the plain landing page still serves on the SF host and carries the
        # same _s.crb token — use it instead of following the redirect.
        land = f"{base_url}/career?company={company}"
        resp = session.get(land, headers=HEADERS, timeout=40)
