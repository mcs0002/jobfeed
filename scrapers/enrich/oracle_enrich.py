...
def description(url: str, session: requests.Session | None = None,
                timeout: int = 20, out: dict | None = None) -> str:
    """Plain-text description for one Oracle CE job URL, or "" on any failure
    (expired requisition, network error, parse error).

    `out`, when given, receives ExternalPostedEndDate as `deadline`. Probed
    on the M1 2026-09-24: 11 of 40 requisitions carried one, mostly 3 to 35
    days out; the one-year defaults beside them are refused by
    plausible_deadline."""
    ...
    item = items[0]
    if out is not None:
        stated = plausible_deadline(item.get("ExternalPostedEndDate"),
