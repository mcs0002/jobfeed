...
from .descriptions import _extract_text, plausible_deadline
...
class WorkdayEnricher:
    ...
    def description(self, public_url: str, tenant: str, board: str,
                    facets: dict | None = None, out: dict | None = None) -> str:
        """Plain-text description for one Workday job, or "" on any failure.

        `out`, when given, receives the posting's `endDate` as `deadline`:
        the date Workday shows candidates as "time left to apply". Probed on
        the M1 2026-09-24: 27 of 40 postings carried one, 6 to 65 days after
        posting with no fixed gap, so an application window rather than an
        automatic expiry."""
        ...
        try:
            ...
            return ""
        if out is not None:
            stated = plausible_deadline(info.get("endDate"), info.get("startDate"))
            if stated:
                out["deadline"] = stated
