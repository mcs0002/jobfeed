...
runtime question answered by ``jobfeed/description_health.py`` (which reads the live DB
...
DESCRIPTION_STRATEGY = {
    ...
    "ossiam": SCRAPER,
    "metzler": HTTP,  # flight payload has no body; detail pages are server-rendered
    "generali": SCRAPER,
    ...
    "teamtailor": HTTP,     # listing-only handler; public detail pages carry real bodies
    "lever": SCRAPER,
    "citadel": SCRAPER,
    "radancy": SCRAPER,
    "janestreet": SCRAPER,
    ...
    "wp_job": SCRAPER,
    "intervieweb": SCRAPER,
    "intervieweb_career": HTTP,  # zinrec detail pages server-render the body
    "nuernberger": SCRAPER,   # ES doc content[] carries tasks/quals inline
    "ww": SCRAPER,            # list API `content` field carries the body
    "provinzial": HTTP,       # TYPO3 detail pages server-render
    "alh": NONE,              # P&I bewerber-web SPA — JS shell, listing-only
    "vig": SCRAPER,           # taskText/profileText inline
    "dvinci": HTTP,           # d.vinci detail pages server-render
    "rwe": SCRAPER,
