...
def main():
    ...
    def _route(row) -> str:
        ...
        if (detail_fn := next((d for is_fn, d in detail_enrichers
            ...
            # Empty from a URL-matched enricher is not the last word — the
            # SuccessFactors matcher is broad enough to claim any
            # `careers.*/…/job/<a>/<b>`, so it swallowed every Radancy row
            # (Munich Re, ERGO, MEAG) and returned "" while the generic lane
            # below reads their JSON-LD at full length. Fall through.
            if text := detail_fn(row["url"], session):
                return text
