...
def scrape(base_url: str, id_scope: str = "") -> list[dict]:
    """`id_scope` namespaces row ids per tenant. Job numbers are per-tenant,
    so the bare `icims_<n>` id lets two tenants collide on one primary key,
    and the second firm's role is silently skipped as already seen. The three
    tenants wired before 2026-09-23 keep bare ids (re-keying them would
    orphan their stored rows' status and stars); every new tenant sets one."""
    ...
    for page in range(MAX_PAGES):
        ...
        for a in soup.find_all("a", href=True):
            ...
            if job_id in jobs:
                continue
            # Screen-reader label ("Job Title" / "Title") is markup, not title.
            for label in a.select(".field-label, .sr-only"):
                label.decompose()
            ...
            href = a["href"].split("?")[0]
            jobs[job_id] = {
                "id": (f"icims_{id_scope}_{job_id}" if id_scope
                       else f"icims_{job_id}"),
                "title": title,
                "url": href,
                "location": "",
            }
