...
PAGE_SIZE = 10
# Pause before re-fetching a listing page that came back with zero cards.
...
def scrape(config: dict) -> list[dict]:
    ...
    for n in range(2, last_page + 1):
        ...
        if not jobs:
            # A 200 with zero cards on a page the total says must be populated
            # is Citadel's cache serving an empty render, not the end of the
            # listing. Seen 2026-08-19: page 5 of 5 came back empty, so the run
            # collected exactly 40 of 48 and tripped the completeness raise
            # below, while an immediate re-fetch returned all 8. Retry once
            # before believing it.
