    ...
    while True:
        ...
        for item in payload["items"]:
            ...
            jobs[job_id] = {
                "id": f"ossiam_{job_id}",
                "title": title,
                # The source exposes no detail URL. A unique fragment keeps
                # storage/read-side identity distinct while still opening the
                # same human careers page (fragments are client-side only).
                "url": f"{CAREERS_URL}#job-{job_id}",
