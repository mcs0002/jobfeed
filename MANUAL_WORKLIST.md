# Manual sources worklist (41 firms)
...
## Bot-check walls (1) — not ours to solve

- **Bank of America (Campus)** — `bankcampuscareers.tal.net` answers with an
  Oleeo Protect "Quick Check Needed" proof-of-work page (ALTCHA) instead of the
  vacancy list, from 2026-09-13; plain `requests` and `curl_cffi` both get it.
  Manual since 2026-09-14, `board_url` kept for a one-line revert. Check the
  campus site by hand. Nomura (Campus) hit the same page on 2026-09-13 but
  cleared the next day, so it stays on `talnet`; `talnet.py` now raises on the
  check, so a relapse at any tal.net board shows as an error rather than an
  empty board. The lateral `Bank of America` Workday source is unaffected.
...
## No scrapable board — email/LinkedIn/prose only (35)
...
**Hedge funds / macro (email-only):** Andurand, Westbeck, Svelland, KLI, Rokos
Capital Management (former Kernel board removed; careers email only,
2026-08-24).
