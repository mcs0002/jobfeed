...
Pure Python, no LLM — cheap to run idle. Wired into ``jobfeed/selfcheck.py`` so the
weekly health run alerts on degraded sources; also runnable directly:

    python3 -m jobfeed.description_health            # full per-ATS table
    python3 -m jobfeed.description_health --alert    # only degraded sources; exit 1 if any
"""
import argparse
import json
import os
...
def per_ats_health(db_path: str = DEFAULT_DB, root: Path = ROOT) -> list[dict]:
    ...
    rows = conn.execute(
        ...
        "FROM jobs_with_description WHERE delisted_at IS NULL"
    ).fetchall()
    conn.close()
