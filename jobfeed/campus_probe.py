...
Companion to jobfeed/campus_sweep.py, run after a sweep leaves rows in the
...
edits anything: the URL that belongs in targets.json or docs/GRAD_SCHEMES.md is a
judgement call, and a plausible-looking careers page for the wrong division is
worse than a known gap.

  python -m jobfeed.campus_probe                       # every failed firm in the season
  python -m jobfeed.campus_probe --only KKR --only Bain
  python -m jobfeed.campus_probe --season 2026-09 --json out.json
"""
import argparse
import json
import re
...
from jobfeed import campus_sweep as cs  # noqa: E402
