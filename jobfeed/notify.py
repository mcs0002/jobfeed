...
CLI:
    python -m jobfeed.notify --test            # one canned test message
    python -m jobfeed.notify "subject" "body"  # arbitrary alert
"""
import os
import smtplib
import subprocess
...
def main(argv: list[str]) -> int:
    if len(argv) == 1 and argv[0] == "--test":
        ok = send_alert("test alert", "This is a test alert from jobfeed/notify.py.")
    ...
    if len(argv) == 2:
        ...
        return 0 if ok else 1
    print(
        'usage: jobfeed/notify.py --test | jobfeed/notify.py "subject" "body"',
        file=sys.stderr,
    )
    return 2
