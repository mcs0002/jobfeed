...
quota can't blow up; wired into ops/deliver.sh after jobfeed/enrich_descriptions.py.

Usage:
  python3 -m jobfeed.backfill_tags                  # tag rows with no area yet
  python3 -m jobfeed.backfill_tags --retag-all      # re-tag every row (new taxonomy)
  python3 -m jobfeed.backfill_tags --retag-missing-desc-facets  # nightly desc-facet fill (≤300)
  python3 -m jobfeed.backfill_tags --since-days 90  # limit to recent rows
  python3 -m jobfeed.backfill_tags --limit 1000     # cap (quick first pass)
  python3 -m jobfeed.backfill_tags --chunk 30       # rows per Haiku batch
"""
import argparse
...
from jobfeed.filter import has_experience_wall
...
def patch_desc_lang(db: JobDB, verbose: bool = True) -> int:
    ...
    from jobfeed.lang_detect import detect_language
    from web.descfmt import clean_text

    rows = db.conn.execute(
        "SELECT id, title, description, lang_req FROM jobs_with_description "
...
def _rows(db: JobDB, retag_all: bool, since_days: int | None,
    ...
    sql = f"SELECT {', '.join(_COLS)} FROM jobs_with_description"
    clauses, params = [], []
    ...
    elif not (retag_all or area or title_like or companies):
    ...
    # --company scopes a re-tag to named sources (e.g. every live role at the
    # central banks and IFIs after the 2026-09-23 `economics` area landed).
    if companies:
        clauses.append(f"company IN ({','.join('?' * len(companies))})")
        params.extend(companies)
    if area or title_like or companies:
...
def main() -> None:
    ...
    ap.add_argument("--company", action="append",
                    help="Re-tag only live rows of this exact source name (repeatable)")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            for j in chunk:
                ...
                # only for rows tagged WITHOUT a description (jobfeed/main.py) — every row
