...
# USD per 1M tokens, as (off_peak, peak). A model absent here is reported in
# tokens only rather than priced from memory, so the cost column reads "$?"
# until its rates are added — which is what it did for every night between the
# move to Gemini and 2026-09-23, since only the DeepSeek rows were listed.
#
# The DeepSeek entries stay: tag_runs.jsonl is append-only and still holds the
# nights that ran on it, and those rows have to keep pricing correctly.
#
#   deepseek-*      api-docs.deepseek.com/quick_start/pricing, read 2026-08-20.
#                   Off-peak is exactly half of peak, hence the pair.
#   gemini-2.5-flash  ai.google.dev/gemini-api/docs/pricing, read 2026-09-23:
#                   $0.30 / 1M input (text, image, video), $0.03 / 1M cached,
#                   $2.50 / 1M output. Google states no time-of-day variation,
#                   so both halves of each pair are the same number and the
#                   peak-hour lookup below is a no-op for it.
RATES = {
    ...
    "gemini-2.5-flash": {"hit": (0.03, 0.03), "miss": (0.30, 0.30), "out": (2.50, 2.50)},
}
# DeepSeek's peak window. Only models whose RATES pair differs are affected.
