...
# this many days, the nightly ops/deliver.sh scan has been silently dying and nothing
    ...
    try:
        ...
        if last_seen_dt < cutoff:
            ...
            return (f"STALE SCAN: MAX(last_seen) in jobs.db is {age_days} day(s) old "
                    ...
                    f"The nightly ops/deliver.sh scan may have been silently failing.")
    except Exception as exc:
        print(f"WARN: selfcheck: freshness check failed: {exc!r}", file=sys.stderr)
    return None


# Publish-freshness threshold. publish_shared.sh runs daily after the scan and now
# emails on every abort, so this is the backstop for the case an abort email
# cannot cover: the job not running AT ALL (agent unloaded, machine asleep,
# plist removed). 3 days tolerates a couple of missed nights before alerting.
PUBLISH_MAX_DAYS = 3
PUBLISH_LOG = Path.home() / "Library/Logs/jobs-publish.log"
_PUBLISH_OK_RE = re.compile(r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})\s+=== publish done",
                            re.M)


def _check_publish_freshness(log_path: Path = PUBLISH_LOG) -> str | None:
    """Warn if the shared jobs.db on R2 looks stale, None if fresh or absent.

    Reads the publish log's last '=== publish done' line rather than asking R2:
    no network, no rclone on selfcheck's launchd PATH, and no credentials in a
    second place. The log is written by the only thing that uploads, so its
    last success is a faithful proxy for the object's age.

    Returns None when the log is missing — a machine that has never published
    is not a machine with a stale publish.
    """
    ...
    if not log_path.exists():
        return None
    try:
        matches = _PUBLISH_OK_RE.findall(log_path.read_text(errors="replace"))
        if not matches:
            return (f"R2 PUBLISH: no successful publish has ever been recorded in "
                    f"{log_path}. The shared jobs.db may never have been uploaded.")
        # Local-time stamps, matching the log() format in publish_shared.sh.
        last = datetime.strptime(matches[-1], "%Y-%m-%d %H:%M:%S")
        age = datetime.now() - last
        if age > timedelta(days=PUBLISH_MAX_DAYS):
            return (f"STALE R2 PUBLISH: last successful publish was "
                    f"{age.days} day(s) ago ({matches[-1]}). The shared "
                    f"jobs.db on R2 is out of date; check {log_path}.")
    except Exception as exc:
        print(f"WARN: selfcheck: publish freshness check failed: {exc!r}",
              file=sys.stderr)
    return None
...
def main() -> int:
    ...
    try:
        proc = subprocess.run(
            [PY, "-m", "jobfeed", "--verify", "--verified-only"],
    ...
    stub_degraded = set()
    try:
        from jobfeed import description_health
    ...
    other_heavy = {}
    try:
        from jobfeed import tag_health
    ...
    link_dead = set()
    try:
        from jobfeed import link_health
    ...
    # BUG 4: Scan-freshness check. If ops/deliver.sh has been silently dying every
    ...
    warns = [w for w in (_check_scan_freshness(DB_FILE),
                         _check_publish_freshness()) if w]
    for w in warns:
        print(f"WARN: selfcheck: {w}", file=sys.stderr)
    # Kept as one string so the existing alert paths below stay unchanged.
    freshness_warn = "\n\n".join(warns)
    ...
    if alerts:
        ...
        if os.environ.get("SELFCHECK_EMAIL"):
            try:
                from jobfeed import notify
                notify.send_alert("selfcheck: staleness detected", freshness_warn)
            except Exception as e:
                print(f"WARN: selfcheck: notify failed: {e!r}", file=sys.stderr)
