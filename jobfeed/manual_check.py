...
Most manual targets expose no public vacancy board at all and route hiring
through email, LinkedIn, or prose-only programme pages. A small remainder uses
a blocked or unsupported system. Koch is automated separately with a captured
WAF cookie; see docs/KOCH_CAPTURE.md.

Usage:
    python -m jobfeed.manual_check            # print the list (markdown)
    python -m jobfeed.manual_check --md FILE  # also write it to FILE
