...
Mechanics: configured API primary with CLI/API fallbacks, jobs
batched (BATCH_SIZE=13), one pipe-delimited line per job, with sub-batch/single
...
from jobfeed.claude_cli import (claude_bin, CLAUDE_BIN_CANDIDATES, NO_TOOLS_ARGS,
                        warm_auth)
...
# filterable as "English only" just because the model skipped the rubric line.
from jobfeed.lang_detect import detect_language as _detect_language
...
TIMEOUT_SECONDS = 180
BATCH_SIZE = 13
# Excerpt char budgets fed to the model per role. The opening carries the role
...
# Auth/credit death signatures. These arrive on the CLI's STDOUT (not stderr)
# with a non-zero exit, so they never reached the failure_reasons text either —
# deliver.log just said "unknown error".
# "session limit" is the plan-quota wording (measured 2026-08-20 05:28 on the
# M1: "You've hit your session limit - resets 6:10am (Europe/Berlin)").
# It matched none of the signatures below, so the CLI was never marked dead,
# the fallback never engaged, and the nightly re-tag aborted on three blank
# chunks instead. Keep this list wider than feels necessary: a missed signature
# costs a whole run, a spurious match only routes to the fallback transport.
_AUTH_DEATH_RE = re.compile(
    r"OAuth session expired|Failed to authenticate|requires usage credits|"
    r"usage limit reached|credit balance is too low|Invalid API key|"
    r"session limit|rate limit|limit reached|"
    r"upgrade to (?:Max|Pro)|out of (?:credit|quota)",
    re.IGNORECASE,
)
# Process-wide sticky: once any batch has proven the CLI's auth is dead, every
# later batch in this process routes straight to the API fallback. It has to be
# process-wide rather than per-health-dict because backfill_tags calls tag_jobs
# once per chunk of BATCH_SIZE rows — one batch per call — so the per-call
# consecutive-blank breaker below can never reach CIRCUIT_BREAKER_THRESHOLD and
# the fallback was unreachable from the nightly re-tag entirely.
_CLI_DEAD_LOCK = threading.Lock()
_CLI_DEAD = False


# Same idea as _CLI_DEAD for the API transport. A provider can fail for reasons
# that will not fix themselves mid-run (402 no credit, 401 bad key, host down),
# and a PRIMARY transport that hard-fails must not blank every tag in the run —
# it has to hand the batch back to the CLI. Learned the moment it mattered:
# DeepSeek answered 402 while TAG_PROVIDER=api was already live.
_API_DEAD = False


def _mark_api_dead() -> None:
    global _API_DEAD
    with _CLI_DEAD_LOCK:
        _API_DEAD = True


def _api_transport_is_dead() -> bool:
    with _CLI_DEAD_LOCK:
        return _API_DEAD


def _mark_cli_dead() -> None:
    global _CLI_DEAD
    with _CLI_DEAD_LOCK:
        _CLI_DEAD = True


def _cli_is_dead() -> bool:
    with _CLI_DEAD_LOCK:
        return _CLI_DEAD
...
AREAS = {
    "markets", "quant", "research", "economics", "ibd", "capital-markets",
...
# A positive managerial marker in the TITLE. seniority='manager' hides a role,
# so it requires evidence: the model's unsupported judgement is not enough.
# deepseek-v4-pro assigns manager to "Risk Review Actuary", "Claims Analyst",
# "Sales, China" and "Legal Advisor Derivatives" — none of which say anything
# managerial — which hid 14 extra roles per 500 (measured 2026-08-19).
_MANAGER_TITLE_MARKER_RE = re.compile(
    # Plurals matter: boards post "Corporate Transformation Directors".
    r"\bmanagers?(?:in)?\b|\bmanagerial\b|\bmgrs?\b|\bheads?\b"
    r"|\bdirectors?\b|\bchiefs?\b|\bsupervisors?\b"
    # `leiter` deliberately has NO left boundary: German stacks the rung onto
    # the front of the noun (Teamleiter, Projektleiter, Abteilungsleiter), and
    # \bleiter\b matches none of them.
    r"|leiter(?:in)?\b|\bteamlead\b|\bteam\s+lead\b"
    # `leader` (not bare `lead`): "Team Leader", "Program Leader" are rungs,
    # but "Lead Engineer" / "Lead Analyst" is a senior IC, not a manager.
    # Surfaced by the Review page 2026-08-20 — "Team Leader Treasury Operations
    # & Payments" was being cleared because \bteam\s+lead\b needs a boundary
    # after "lead", which "Leader" does not give.
    r"|\bleaders?\b|\bl[ií]der(?:es)?\b"
    r"|\bchef(?:in)?\s+de\b|\bresponsable\b|\bvorstand\b",
    re.IGNORECASE,
)
...
def _enforce_manager(job: dict) -> None:
    """Reconcile seniority with what 'Manager' actually means in the title.

    Two directions, both deterministic, because seniority='manager' is a HARD
    GATE — the browse query is `... AND seniority != 'manager'`, so a wrong
    manager label does not merely mislabel a role, it HIDES it.

    UP: a managerial-RUNG 'Manager' forces seniority=manager (hidden behind the
    same show_senior toggle as YoE-walled roles). Never overrides intern.

    DOWN: if every 'Manager' in the title is a role NOUN ('Asset Manager',
    'Product Manager', 'Relationship Manager' — a job kind, not a rung) then
    'manager' is not a defensible rung and is cleared to '' (unclassified,
    therefore still visible). Measured 2026-08-19: this is deepseek-v4-flash's
    single biggest disagreement with Haiku — it read the noun as the rung on
    Asset/Product/Relationship Manager titles and would have hidden every one
    of them. The guard is model-agnostic; Haiku can make the same mistake.
    """
    title = job.get("title") or ""
    # Strip the role-noun phrases; any 'manager' still standing is a real rung.
    residual = _MANAGER_ROLE_NOUN_RE.sub(" ", title)
    if _MANAGER_RE.search(residual):
        if job.get("seniority") in ("", "graduate", "analyst", "associate"):
            job["seniority"] = "manager"
        return
    if job.get("seniority") != "manager":
        return
    # Reaching here means no managerial RUNG survived the role-noun strip. If
    # the title carries no managerial marker at all either, the label rests on
    # nothing checkable — and the cost of being wrong is asymmetric: a false
    # manager HIDES a role, a false non-manager merely shows one. Clear it.
    if not _MANAGER_TITLE_MARKER_RE.search(title):
        job["seniority"] = ""
    elif _MANAGER_RE.search(title):
        # All 'manager' occurrences were role nouns (Asset/Product Manager).
        job["seniority"] = ""

_OUTPUT_MARKER = "INPUT: numbered job blocks separated by ===."
...
- research         : sell-side or independent INVESTMENT research — analysts and strategists covering equity/credit/macro/FX/rates/commodities for investor clients. NOT economists (→ economics). NOT market / consumer / UX / academic research.
- economics        : economists and economic / policy analysis — economic research and forecasting (incl. a bank's economics team), monetary / fiscal / financial-stability policy, economic statistics, competition / antitrust and regulatory economics, and economic consulting (analyst, associate, economist at an economic consultancy). At central banks, MDBs, IFIs and international organisations: economist, policy analyst, economic research analyst, research assistant in an economics department. NOT an investment strategist selling a market view (→ research); NOT management / strategy consulting (→ consulting).
...
- Consulting & Advisory → expect consulting (strategy, deal/transaction advisory, restructuring, valuation); economic consulting (competition/antitrust, regulatory, damages, economic analysis) → economics; audit/tax/assurance/bookkeeping → accounting; the usual tech/HR/ops noise → other. A clearly corporate-finance/M&A-advisory desk → ibd.
...
- MDBs, Central Banks & IFIs → economists, policy and economic research → economics; investment, treasury, market operations → markets / asset-management; supervision of banks → risk; lending and project finance → corporate-banking or debt. IT, HR, admin, facilities → other.
...
# Content-derived rather than manually bumped: a prompt/rubric edit creates a
# new evaluation cohort automatically and cannot be forgotten during deploy.
RUBRIC_VERSION = hashlib.sha256(_SYSTEM.encode("utf-8")).hexdigest()[:12]


# ── Structured-output mode (opt-in, TAG_API_FORMAT) ──────────────────────────
# The pipe format is cheapest and remains the production default. Some models
# intermittently add or drop fields despite the prompt; constrained JSON makes
# the row and field counts structural rather than advisory.
_RUBRIC_PREFIX = _SYSTEM.split(_OUTPUT_MARKER)[0]

_FIELD_ORDER = ("index", "area", "desk", "seniority", "type", "city", "country",
                "region", "work_mode", "lang_req", "min_yoe", "education",
                "start_date")

# The seen_jobs columns the eval scripts score a tagging run on. scripts/
# tag_gold.py (accuracy against the versioned gold set) and scripts/tag_ab.py
# (drift against the incumbent) each kept their own copy of this tuple; a facet
# added to one and not the other would have been scored by one benchmark only.
SCORED_FIELDS = ("area", "desk", "seniority", "job_type", "loc_country", "work_mode")

_OUTPUT_JSON_ROWS = """INPUT: numbered job blocks separated by ===.
OUTPUT: JSON only, matching the supplied schema. `rows` holds ONE inner array per input job, in the SAME order as the input, one for EVERY job including the ones you are unsure about.
Each inner array is EXACTLY 13 strings in this order:
INDEX, AREA, DESK, SENIORITY, TYPE, CITY, COUNTRY, REGION, WORKMODE, LANG_REQ, MIN_YOE, EDUCATION, START_DATE
Output "-" for any field that is empty or not applicable. MIN_YOE is always a numeric string ("0" when none).

Examples of single inner arrays:
["0","markets","trading","graduate","graduate-programme","London","United Kingdom","Europe","onsite","-","0","bachelor","2026-09"]
["1","other","-","analyst","job","Essen","Germany","Europe","onsite","de","0","-","-"]
["2","quant","-","analyst","job","Hong Kong","Hong Kong","APAC","hybrid","-","3","phd","asap"]
["3","asset-management","-","associate","job","Geneva","Switzerland","Europe","onsite","fr","5","master","-"]
(Row 0's `2026-09` came from a stated "September 2026" intake.)
"""

_OUTPUT_JSON_OBJ = """INPUT: numbered job blocks separated by ===.
OUTPUT: JSON only, matching the supplied schema. `rows` holds ONE object per input job, in the SAME order as the input, one for EVERY job including the ones you are unsure about.
Each object has exactly these keys: index, area, desk, seniority, type, city, country, region, work_mode, lang_req, min_yoe, education, start_date.
`index` is the job's INDEX as a string. `min_yoe` is an integer (0 when none). Every other value is a string; output "-" for any field that is empty or not applicable.

Example object:
{"index":"0","area":"markets","desk":"trading","seniority":"graduate","type":"graduate-programme","city":"London","country":"United Kingdom","region":"Europe","work_mode":"onsite","lang_req":"-","min_yoe":0,"education":"bachelor","start_date":"2026-09"}
(That `2026-09` came from a stated "September 2026" intake.)
"""

_RUBRIC_ADDENDUM = """
BOUNDARY CHECK — apply after you have read the description, before you commit to AREA. These are the cases where a finance-sounding title sits on top of an excluded function. The function decides, never the title:
- Settling, confirming, reconciling or clearing trades is BACK-OFFICE => other, however senior the instrument sounds. Only product/valuation control, trade support, collateral management and business-management/COO work are middle-office.
- Compliance, monitoring, surveillance, governance, control testing and regulatory reporting are SUPPORT => other, including when the subject is a portfolio, a fund or an investment.
- Building, architecting, implementing or transforming systems is TECH => other, including at a consultancy and including ERP/EPM/platform work. Consulting means advising on the business, not delivering the software.
- Investor relations, client reporting, fund administration and marketing at an investment firm are SUPPORT => other. The private-equity, debt and asset-management areas mean INVESTING the capital.
- Serving individual retail customers in a branch or contact centre is RETAIL => other. Wealth means advising the assets of wealthy individuals and families.
- Talent pools, insight days, business courses, discovery events and expressions of interest are NOT OPENINGS => other, type=job.
When the description gives you no evidence either way, keep the label the title implies.
"""

_SYSTEM_JSON_ROWS = _RUBRIC_PREFIX + _OUTPUT_JSON_ROWS
_SYSTEM_JSON_OBJ = _RUBRIC_PREFIX + _OUTPUT_JSON_OBJ

_AREA_VALUES = sorted(AREAS)
_DESK_VALUES = sorted(DESKS) + ["-"]
_SENIORITY_VALUES = sorted(SENIORITIES)
_TYPE_VALUES = sorted(JOB_TYPES)
_REGION_VALUES = sorted(REGIONS) + ["-"]
_WORKMODE_VALUES = sorted(WORK_MODES)
_EDUCATION_VALUES = sorted(EDUCATION_LEVELS) + ["-"]


def _json_schema(fmt: str, count: int) -> dict:
    """Return a schema that permits exactly one output row per input row."""
    if fmt == "json_obj":
        props = {
            "index": {"type": "string", "pattern": "^[0-9]+$"},
            "area": {"type": "string", "enum": _AREA_VALUES},
            "desk": {"type": "string", "enum": _DESK_VALUES},
            "seniority": {"type": "string", "enum": _SENIORITY_VALUES},
            "type": {"type": "string", "enum": _TYPE_VALUES},
            "city": {"type": "string"},
            "country": {"type": "string"},
            "region": {"type": "string", "enum": _REGION_VALUES},
            "work_mode": {"type": "string", "enum": _WORKMODE_VALUES},
            "lang_req": {"type": "string"},
            "min_yoe": {"type": "integer"},
            "education": {"type": "string", "enum": _EDUCATION_VALUES},
            "start_date": {"type": "string"},
        }
        item = {"type": "object", "properties": props,
                "required": list(_FIELD_ORDER),
                "additionalProperties": False,
                "propertyOrdering": list(_FIELD_ORDER)}
    else:
...
def _json_to_pipe(out_text: str, expected_count: int) -> str:
    """Validate structured rows, then feed them through the shared parser."""
    data = json.loads(out_text)
    rows = data.get("rows", []) if isinstance(data, dict) else data
    if not isinstance(rows, list) or len(rows) != expected_count:
        raise ValueError(
            f"expected {expected_count} structured rows, got "
            f"{len(rows) if isinstance(rows, list) else 'non-list'}"
        )
    lines = []
    for i, row in enumerate(rows):
        if isinstance(row, dict):
            vals = [row.get(k, "-") for k in _FIELD_ORDER]
        else:
            vals = list(row)
        # Collapse whitespace inside every field: the model sometimes ends a
        # value with "\n" ("-\n"), which split each row across lines and the
        # line parser then rejected the whole batch (six 13-role batches in two
        # minutes of the 2026-09-23 re-tag).
        vals = [(" ".join(str(v if v is not None else "").split()) or "-")
                .replace("|", "/") for v in vals]
        if len(vals) != 13:
            raise ValueError(f"row {i} has {len(vals)} fields, expected 13")
        if not vals[0].strip().isdigit() or int(vals[0]) != i:
            raise ValueError(f"row {i} returned invalid index {vals[0]!r}")
        lines.append("|".join(vals))
    return "\n".join(lines)


def _system_for_cfg(cfg: dict) -> str:
    """Return the exact system text used by one configured API request."""
    fmt = cfg.get("format", "pipe")
    text = {"json_rows": _SYSTEM_JSON_ROWS,
            "json_obj": _SYSTEM_JSON_OBJ}.get(fmt, _SYSTEM)
    if _cfg("TAG_RUBRIC_ADDENDUM", "").strip().lower() == "exclusions":
        text += _RUBRIC_ADDENDUM
    return text


def _rubric_version_for_cfg(cfg: dict) -> str:
    """Hash the active prompt, including structured mode and addendum."""
    return hashlib.sha256(_system_for_cfg(cfg).encode("utf-8")).hexdigest()[:12]
...
def _fresh_health() -> dict:
    return {
        ...
        "api_fallback": False,
        # OpenAI-compatible transport accounting (0 on the other transports).
        "tokens_in": 0,
        "tokens_out": 0,
        "tokens_cached": 0,
    }
...
# One process = one run id, so the ~68 tag_jobs calls backfill_tags makes from 4
# threads aggregate back into a single night rather than looking like 68 runs.
_RUN_ID = f"{int(time.time())}-{os.getpid()}"
_RUNS_LOG_LOCK = threading.Lock()


def _record_run(health: dict) -> None:
    ...
    try:
        ...
        rec = {
            ...
            "rubric_version": (_rubric_version_for_cfg(cfg)
                               if cfg else RUBRIC_VERSION),
            "jobs_total": health.get("jobs_total", 0),
            "jobs_tagged": health.get("jobs_tagged", 0),
            "batches_total": health.get("batches_total", 0),
            "batches_ok": health.get("batches_ok", 0),
            "batches_failed": health.get("batches_failed", 0),
            "retries": (health.get("subbatch_retries", 0)
                        + health.get("single_retries", 0)),
            "tokens_in": health.get("tokens_in", 0),
            "tokens_cached": health.get("tokens_cached", 0),
            "tokens_out": health.get("tokens_out", 0),
            "latency_s": round(health.get("total_latency_s", 0.0), 1),
            # The three ways a run can silently stop being what you configured.
            "cli_down": bool(health.get("cli_down")),
            "api_fallback": bool(health.get("api_fallback")),
            "api_transport_down": bool(health.get("api_transport_down")),
        }
        line = json.dumps(rec, ensure_ascii=False) + "\n"
        # Resolved from ROOT at call time, not the module constant, so the test
        # suite's patched ROOT keeps its records out of the real repo.
        with _RUNS_LOG_LOCK:
            with open(os.path.join(ROOT, "tag_runs.jsonl"), "a") as fp:
                fp.write(line)
    except Exception:
        pass
...
def _blank_tags(job: dict) -> None:
    ...
    job["education"] = None
    job["start_date"] = None
    # min_yoe stays absent on a blank tag — jobfeed/main.py/backfill fall back to the
...
def _tag_batch(batch: list[dict], bin_path: str, health: dict | None = None) -> None:
    ...
    if result.returncode != 0:
        err = ((result.stderr or "").strip()
               ...
               or "unknown error")[:200]
        reason = f"claude CLI exit {result.returncode}: {err}"
        if _AUTH_DEATH_RE.search((result.stdout or "") + (result.stderr or "")):
            _mark_cli_dead()
        for j in batch:
            _blank_tags(j)
        if health is not None:
...
def _cfg(name: str, default: str = "") -> str:
    """Read `name` from the environment, else from the repo .env. launchd hands
    the scan almost no environment, so .env is the real config surface here."""
...
def _api_key() -> str:
    """ANTHROPIC_TAG_API_KEY — the paid Console account. Only ever a FALLBACK
    behind the subscription CLI; never the primary."""
    return _cfg("ANTHROPIC_TAG_API_KEY")


# ── OpenAI-compatible transport ───────────────────────────────────────────────
# The third transport, and the only one with no Anthropic dependency: any host
# speaking /v1/chat/completions (DeepSeek, Together, Groq, OpenRouter, a local
# Ollama) works from config alone. Set TAG_PROVIDER=api to make it PRIMARY,
# which is the point — it retires the OAuth failure mode entirely, because
# there is no CLI, no refresh and no launchd auth in the path.
#
# Prefix caching is why the system prompt stops mattering: it is ~4.5k tokens
# resent on every 13-role batch (~35% of all input), and every provider worth
# using bills a cache hit at a fraction of a miss — DeepSeek at $0.007/1M
# against $0.22 (checked 2026-08-19). The system prompt is already the first
# thing in every request, so the hit comes for free; do not reorder it.
def _openai_cfg() -> dict:
    # TAG_API_EXTRA is a JSON object merged into the request body. It exists so
    # the transport can stay vendor-neutral while still reaching provider knobs
    # that matter — the first and most important being DeepSeek's `thinking`.
    # A reasoning model is exactly wrong for this job: the rubric does the
    # thinking, we want format-compliant lines, and v4-flash spent its whole
    # 4k output budget reasoning and returned an EMPTY content field (measured
    # 2026-08-19: 24,576 output tokens, zero parseable rows).
    extra = {}
    raw = _cfg("TAG_API_EXTRA")
    if raw:
        try:
            ...
            extra = {}
    return {
        "base_url": _cfg("TAG_API_BASE_URL").rstrip("/"),
        "api_key": _cfg("TAG_API_KEY"),
        "model": _cfg("TAG_API_MODEL"),
        "max_tokens": int(_cfg("TAG_API_MAX_TOKENS", "4096")),
        "extra": extra if isinstance(extra, dict) else {},
        # pipe (default) | json_rows | json_obj
        "format": _cfg("TAG_API_FORMAT", "pipe").strip().lower(),
    }


# TAG_PROVIDER values. "api" is the documented spelling; "openai"/"http" are
...
def tag_provenance() -> dict[str, str]:
    ...
    provider = _provider()
    cfg = _openai_cfg() if provider == "api" else {}
    model = cfg.get("model", "") if cfg else MODEL
    return {"provider": provider, "model": model or MODEL,
            "rubric_version": (_rubric_version_for_cfg(cfg)
                               if cfg else RUBRIC_VERSION)}


# Retries for the two HTTP transports. Any exception used to mark the primary
# API dead for the rest of the process, so one timeout or one 503 from Gemini
# sent the remaining night to the subscription CLI and fired an alert. Only a
# failure that can clear on its own is retried; a 400/401/402/403 still fails
# at once, because waiting cannot fix a bad key or an empty balance.
API_RETRY_DELAYS = (2, 8)  # seconds before the 2nd and 3rd attempt
...
def _post_json(url: str, *, headers: dict, body: dict, label: str) -> dict:
    """POST and decode JSON, retrying timeouts, connection errors and
    transient statuses. Raises the last error once the retries are spent."""
    import requests
    ...
    for delay in (*API_RETRY_DELAYS, None):
        try:
            ...
            if status in _TRANSIENT_STATUS and delay is not None:
                raise _TransientHTTP(f"HTTP {status}")
            resp.raise_for_status()
            return resp.json()
        except (requests.Timeout, requests.ConnectionError, _TransientHTTP) as exc:
            if delay is None:
                raise
            _log_debug(f"{label}-RETRY in {delay}s", "",
                       f"{type(exc).__name__}: {exc}")
            time.sleep(delay)
    raise AssertionError("unreachable")


def _tag_batch_openai(batch: list[dict], cfg: dict,
                      health: dict | None = None) -> None:
    """Tag one batch through an OpenAI-compatible /v1/chat/completions host.
    Same payload, same system prompt and the same parser as the other two
    ...
    body = {
        ...
        "messages": [{"role": "system", "content": _system_for_cfg(cfg)},
                     {"role": "user", "content": payload}],
        "temperature": 0,
        "stream": False,
    }
    # Provider-specific extras override defaults; null removes a default key.
    for key, val in (cfg.get("extra") or {}).items():
        if val is None:
            body.pop(key, None)
        else:
            body[key] = val
    if fmt in ("json_rows", "json_obj"):
        body["response_format"] = {
            "type": "json_schema",
            "json_schema": {"name": "job_tags", "strict": True,
                            "schema": _json_schema(fmt, len(batch))},
        }
    start = time.monotonic()
    try:
        data = _post_json(
        ...
        if not out_text:
            # An empty content field with tokens billed means the model spent
            # the budget somewhere else (reasoning) or was cut off. Say which —
            # "(empty)" in the debug log tells you nothing.
            _log_debug(
                f"API-EMPTY-CONTENT batch={len(batch)}",
                f"finish_reason={choice.get('finish_reason')!r} "
                f"reasoning_chars={len(msg.get('reasoning_content') or '')} "
                f"usage={data.get('usage')}",
                "")
        if health is not None and (usage := data.get("usage")):
            health["tokens_in"] = health.get("tokens_in", 0) + usage.get("prompt_tokens", 0)
            health["tokens_out"] = health.get("tokens_out", 0) + usage.get("completion_tokens", 0)
            # Providers spell the cache-hit counter differently; record whatever
            # is offered so a run can prove the prefix cache is working.
            det = usage.get("prompt_tokens_details") or {}
            hit = det.get("cached_tokens", usage.get("prompt_cache_hit_tokens", 0))
            health["tokens_cached"] = health.get("tokens_cached", 0) + (hit or 0)
    except Exception as exc:
        reason = f"openai transport failed: {type(exc).__name__}: {exc}"
        for j in batch:
            _blank_tags(j)
        if health is not None:
            health["batches_failed"] += 1
            ...
            health["total_latency_s"] += time.monotonic() - start
        _mark_api_dead()
        _log_debug(f"API-TRANSPORT-FAIL batch={len(batch)}", "", reason)
        return

    if health is not None:
        health["total_latency_s"] += time.monotonic() - start
    if fmt in ("json_rows", "json_obj") and out_text:
        try:
            out_text = _json_to_pipe(out_text, len(batch))
        except (ValueError, TypeError, AttributeError) as exc:
            _log_debug(f"JSON-DECODE-FAIL batch={len(batch)}", out_text,
                       f"{type(exc).__name__}: {exc}")
            out_text = ""
    ...
    try:
        data = _post_json(
            ...
            body={
                "model": MODEL,
                "max_tokens": 4096,
                # Cache the ~4.5k-token rubric. It is byte-identical on every
                # call and is ~40% of the input, so a cache read at 0.1x turns
                # the largest single line item into a rounding error. Measured
                # on the OpenAI transport, whose provider does this
                # automatically: 99% of input tokens came back cached.
        ...
        if health is not None and (usage := data.get("usage")):
            health["tokens_in"] = health.get("tokens_in", 0) + usage.get("input_tokens", 0)
            health["tokens_out"] = health.get("tokens_out", 0) + usage.get("output_tokens", 0)
            health["tokens_cached"] = (health.get("tokens_cached", 0)
...
def _announce_fallback(health: dict | None) -> None:
    """Flip the run onto the paid API and say so ONCE, loudly.

    The breaker path below prints this; the sticky-flag path above did not, so
    a run could switch to the billed Console account with deliver.log showing
    nothing at all — which is exactly what happened on 2026-08-19 (the alert
    email fired, the log was silent, and the log is where you look first)."""
    if health is None:
        return
    if not health.get("api_fallback"):
        print("\nWARN: claude CLI appears down — switching to direct-API "
              "fallback (ANTHROPIC_TAG_API_KEY)", flush=True)
    health["api_fallback"] = True


def _tag_batch_any(batch: list[dict], bin_path: str | None,
                   health: dict | None = None) -> None:
    ...
    if _provider() == "api" and not _api_transport_is_dead():
        ...
        if cfg["base_url"] and cfg["api_key"] and cfg["model"]:
            _tag_batch_openai(batch, cfg, health=health)
            # Only keep the result if it worked. A dead primary (no credit,
            # bad key, host down) must fall through to the CLI below, not
            # blank the run — the whole point of having three transports.
            if not _api_transport_is_dead():
                return
            if health is not None and not health.get("api_transport_down"):
                health["api_transport_down"] = True
                if bin_path:
                    print("\nWARN: TAG_PROVIDER transport failed — falling back "
                          "to the claude CLI for the rest of this run", flush=True)
                    health["failure_reasons"].append(
                        "configured API transport failed; fell back to the CLI")
                else:
                    health["failure_reasons"].append(
                        "configured API transport failed; no claude CLI fallback "
                        "is installed")
        # Misconfigured: say so once, then behave exactly as before.
        if health is not None and not health.get("openai_misconfigured"):
            health["openai_misconfigured"] = True
            fallback = ("the claude CLI" if bin_path else
                        "the Anthropic API" if _api_key() else
                        "no working fallback")
            health["failure_reasons"].append(
                "TAG_PROVIDER=api but TAG_API_BASE_URL/_API_KEY/_MODEL "
                f"are not all set — falling back to {fallback}")
    if _provider() == "api" and not bin_path and (key := _api_key()):
        if health is not None and not health.get("api_fallback"):
            print("\nWARN: configured API transport unavailable and no claude "
                  "CLI installed — falling back to Anthropic Messages API",
                  flush=True)
            health["api_fallback"] = True
        _tag_batch_api(batch, key, health=health)
        return
    if (health is not None and health.get("api_fallback")) or _cli_is_dead():
        key = _api_key()
        if key:
            _announce_fallback(health)
            _tag_batch_api(batch, key, health=health)
            return
    if not bin_path:
        # A configured OpenAI-compatible provider is fully standalone. We land
        # here only when that provider is missing/dead and neither the Claude
        # CLI nor the Anthropic Messages fallback is available.
        for j in batch:
            _blank_tags(j)
        if health is not None and not health.get("cli_missing"):
            health["cli_missing"] = True
            health["failure_reasons"].append(
                "claude CLI not found and no working API transport remained"
            )
        return
    # API-primary runs do not touch OAuth while the API works. Warm lazily only
    # if the configured API failed and this batch is actually falling back.
    if _provider() == "api":
        warm_auth(bin_path, MODEL)
    _tag_batch(batch, bin_path, health=health)
    # That call may be the one that discovered the auth is dead. Recover this
    # batch on the spot rather than leaving it blank for a caller that has no
    # retry left (backfill_tags aborts the whole run after 3 blank chunks).
    if _cli_is_dead() and all(_is_untagged(j) for j in batch):
        key = _api_key()
        if key:
            _announce_fallback(health)
            _tag_batch_api(batch, key, health=health)
...
def tag_jobs(jobs: list[dict]) -> list[dict]:
    ...
    health["cli_path"] = bin_path
    if provider == "cli":
        # One serialized refresh before any fan-out — see
        # claude_cli.warm_auth. API-primary runs deliberately skip this entire
        # OAuth path; they warm only if the API fails and the CLI is needed.
        warm_auth(bin_path, MODEL)
    ...
    # Batches are sent in waves (_tag_wave); everything below judges them one
    # at a time, in order, after the wave returns.
    consecutive_blank = 0
    batches = [jobs[i:i + BATCH_SIZE] for i in range(0, len(jobs), BATCH_SIZE)]
    wave_size = _tag_workers() if provider == "api" else 1
        ...
        if health.get("cli_down"):
            for batch in wave:
                for j in batch:
                    _blank_tags(j)
            continue
        _tag_wave(wave, bin_path, health)
        for batch in wave:
            if health.get("cli_down"):
                break  # tripped earlier in this wave; the rest stand as sent
            consecutive_blank = _judge_batch(batch, bin_path, health,
                                             consecutive_blank)
...
# Concurrent batches per wave on the API transport. Sequential batches were
# the whole tagging wall clock: 954 roles took 310 s on 2026-09-22 at ~4 s a
# call. The CLI path stays at one so its circuit breaker keeps its exact
# semantics (3 consecutive blank batches, no extra doomed subprocesses).
TAG_WORKERS = 4


def _tag_workers() -> int:
    try:
        return max(1, int(_cfg("TAG_WORKERS", str(TAG_WORKERS))))
    except ValueError:
        return TAG_WORKERS


def _worker_health(health: dict) -> dict:
    """Fresh counters for one concurrent batch, carrying the run's sticky
    flags so "say it once" guards inside the transports stay once-ish."""
    h = _fresh_health()
    h.update({k: v for k, v in health.items() if isinstance(v, bool)})
    h["cli_path"] = health.get("cli_path")
    return h


def _merge_health(into: dict, part: dict) -> None:
    for k, v in part.items():
        if k in ("cli_path", "model", "jobs_total", "jobs_tagged"):
            continue
        if isinstance(v, bool):
            ...
            into.setdefault(k, []).extend(x for x in v if x not in into[k])


def _tag_wave(wave: list[list[dict]], bin_path: str | None, health: dict) -> None:
    """Send a wave of batches concurrently. Each worker accounts into its own
    health dict, merged back in batch order, so no counter is shared across
    threads. The process-wide dead-transport flags are already locked."""
    if len(wave) == 1:
        _tag_batch_any(wave[0], bin_path, health=health)
        return
    parts = [_worker_health(health) for _ in wave]
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(wave)) as ex:
        list(ex.map(lambda bp: _tag_batch_any(bp[0], bin_path, health=bp[1]),
                    zip(wave, parts)))
    for part in parts:
        _merge_health(health, part)


def _judge_batch(batch: list[dict], bin_path: str | None, health: dict,
                 consecutive_blank: int) -> int:
    """Breaker, paid-API rescue and retries for one sent batch. Returns the
    updated consecutive-blank count."""
