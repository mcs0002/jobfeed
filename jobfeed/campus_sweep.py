...
listed in ``docs/GRAD_SCHEMES.md`` — but nothing ever *checked* them. A programme
...
docs/GRAD_SCHEMES.md. The autumn window is the load-bearing one: most European
...
picks up where it stopped:
  ...
  2. READ — each cached page goes to the configured LLM transport (the same
     OpenAI-compatible provider jobfeed/tag.py uses, falling back to the claude CLI)
...
Usage:
  python -m jobfeed.campus_sweep                  # run both phases, resumable
  python -m jobfeed.campus_sweep --limit 5        # smoke test on the first 5 firms
  python -m jobfeed.campus_sweep --only "Man Group" --only Optiver
  python -m jobfeed.campus_sweep --fetch-only     # phase 1 only
  python -m jobfeed.campus_sweep --report         # rebuild the markdown from results
  python -m jobfeed.campus_sweep --retry-failed   # re-fetch pages that errored
"""
import argparse
import json
import os
...
# docs/GRAD_SCHEMES.md contributes its curated programme page (which wins on
...
def parse_grad_schemes() -> list[dict]:
    """Rows of every markdown table in docs/GRAD_SCHEMES.md, with their section."""
...
# Order the list is worked in: what closes soonest, first. `jobfeed/campus.py` sorts
# the page with the same order and used to keep its own copy.
STATUS_ORDER = {"open": 0, "opens_later": 1, "unclear": 2, "closed": 3}


def _rank(progs: list[dict]) -> dict:
    return sorted(progs, key=lambda p: STATUS_ORDER.get(p.get("status"), 9))[0]
...
def build_report(results: Path, out_md: Path, today: str) -> str:
    ...
    out = [f"# Campus sweep — {today}", "",
           ...
           "ATS scrape cannot see. Generated by `jobfeed/campus_sweep.py`; every status and "
    ...
    def section(title: str, key: str, blurb: str) -> None:
        ...
        for r in sorted(rs, key=lambda x: (x.get("category", ""), x["name"])):
            ...
            out.append(f"<{r['url']}>  \n_{r.get('why', '')}_")
            for p in sorted(progs, key=lambda x: STATUS_ORDER.get(x.get("status"), 9)):
                meta = _fmt(p)
