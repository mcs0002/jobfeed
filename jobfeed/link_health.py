...
removal (posting pulled between the nightly scan and this check), so a company is
...
Wired into ``jobfeed/selfcheck.py`` (weekly, Sunday 03:00 on the M1, live DB); also
runnable directly:

    python3 -m jobfeed.link_health            # check now, print per-company verdicts
    python3 -m jobfeed.link_health --alert    # only streak-confirmed dead; exit 1 if any
"""
import argparse
import json
import os
...
# (jobfeed/selfcheck.py ~line 184), so a broken install is visible immediately instead
