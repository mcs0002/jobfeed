...
> **Snapshot, 2026-07-06.** The reasoning holds; the counts do not. On
> 2026-09-23 there were ~490 targets and the scan ran four times a day.
> Read the live numbers off the Sources page.
...
scraping personal data at scale. This system does none of the four: a few
polite scans a day per firm (four since 2026-09-23, most boards one JSON request
each), fail-loud instead of retry-hammering, private use, no redistribution. Even the `curl_cffi` TLS impersonation only defeats fingerprint
...
- **Collapse guard** (`jobfeed/selfcheck.py`) — rolling per-source baseline (max jobs
...
- **Never-produced guard** (`jobfeed/selfcheck.py`) — flags a verified source stuck at 0
...
1. **Dead apply links** — ~~the biggest true blind spot~~ **built 2026-07-06**
   (`jobfeed/link_health.py`, wired into selfcheck): samples 2 random live URLs per
...
2. **Tag-quality drift** — ~~found by eye~~ **built 2026-07-06**
   (`jobfeed/tag_health.py`, wired into selfcheck): flags companies with ≥5 tagged
