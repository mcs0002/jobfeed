...
- **`targets.json`** — each unscrapable firm is `"ats": "manual"` with a
  ...
  live here). `python -m jobfeed.manual_check` prints them; the Sources page shows them
  via the ⓘ note.
...
public ATS at all** is a genuine block, and even then the answer is `manual`,
not a browser (the scan keeps a no-Playwright policy). No test enforces this any more:
`tests/test_playwright_allowlist.py` went out with the automated apply
pipeline (e706055). The one browser path left in the scan is implicit: any
Workday board that answers HTTP 422 falls back to `workday_playwright`
(`scrapers/__init__.py`, `_workday`). Koch and Tikehau
...
- Wired 2026-07-04: Bank of America (Campus) 67, Nomura (Campus) 6, Evercore (Campus, vacancy/2) 4,
  Perella Weinberg (Students, vacancy/2) 0-but-valid (seasonal, autumn). Perella Weinberg (Students)
  has since moved to Workday. Bank of America (Campus) sat behind an Oleeo "Quick Check" bot page
  from 2026-09-13 to 2026-09-23 (`manual` meanwhile); `talnet.py` raises on that page.
- **BNP Paribas is the exception — bnpparibas.tal.net is a JS-ONLY Oleeo board.** Every
  ...
  full) — roles_on_lateral. No reason to chase the Oleeo board.** FTI Consulting is now
  scraped through Workday instead.
