...
def _enrich_new_jobs(new_jobs: list[dict], db: JobDB, dry_run: bool) -> None:
    ...
    def _detail_then_http(j: dict, s) -> str:
        """A URL-matched detail enricher coming back empty is NOT proof the page
        has no body. The SuccessFactors matcher claims any
        `careers.*/…/job/<a>/<b>` URL, which is how every Radancy row (Munich
        Re, ERGO, MEAG) sat description-less from July 2026 — the plain-HTML
        lane reads their JSON-LD fine, it just never got a turn. Same collision
        the TalentBrew id-prefix lane above exists to dodge; falling through
        fixes the whole class instead of one ATS at a time."""
        text = detail_enricher(j["url"])(j["url"], s, timeout=ENRICH_TIMEOUT_SECONDS)
        return text or enrich_one(j["url"], s, timeout=_enrich_timeout(j))

    _pool(lanes["detail"], _detail_then_http)
...
def main():
    ...
    if not args.dry_run and checked > 0:
        ...
        # the list AFTER --company filtering. `main.py --company "BIS"` therefore
